    ...
    <div class="card">
        <div class="card-body">
            ...
            <div class="card-title">
                <a href="/networkswitch/create" class="btn btn-primary">Create</a>
            </div>
            @endif
                ...
                <table id="tbldata" class="table table-striped table-bordered table-hover">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $data->switch_type->name }}</td>
                            <td>{{ $data->ports->count() }}</td>
                            <td>{{ $data->location }}</td>
                            <td>                                
                                @if (Auth::user()->role->name == 'Administrator')
                                <a href="/networkswitch/{{ $data->id }}/edit" class="btn btn-sm btn-primary mr-1">Edit</a>
                                <button type="button" class="btn btn-sm btn-danger" onclick="confirmDelete({{ $data->id }});">Delete</button>
                                @else
                                <a href="/networkswitch/{{ $data->id }}" class="btn btn-sm btn-success mr-1">View</a>
                                @endif
                            </td>
                        </tr>
                        @endforeach
    ...
    <script type="text/javascript">
        function confirmDelete(id) {
            bootbox.confirm("Are you sure want to delete?", function(result) {
                if (result) {
                    $('#delete-form').attr('action', '/networkswitch/' + id);
                    $('#delete-form').submit();
                }
            })
        }
